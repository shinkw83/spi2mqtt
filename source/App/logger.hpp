...
class LogMgrC{
		  ...
		  , logger(log_name?Logger::getInstance(log_name):Logger::getRoot())
	{
		std::unique_ptr<Layout> myLayout
			= std::unique_ptr<Layout>(new log4cplus::PatternLayout("%D{%m/%d %H:%M:%S:%q } %-5p %m%n"));

		myAppender->setLayout(std::move(myLayout));
...
class DebugLogger{
	...
	{
		std::unique_ptr<Layout> myLayout
			= std::unique_ptr<Layout>(new log4cplus::PatternLayout("%D{%m/%d %H:%M:%S:%q } %m%n"));
